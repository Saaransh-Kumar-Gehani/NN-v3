## [v3.01] - Neuron Class  |  19/03/26
- Added `Neuron` Class in `neuron.py`.
- Added functions `predict()`, `activate()`, and `save()` to the class.


## [v3.02] - Layers Class  |  21/03/26
- Added `Layers` Class in `layers.py`.
- Created a `config.json` and `data.json` file in `Files\`.


## [v3.03] - Trainer Class  |  25/03/26
- Started `Trainer` Class in `trainer.py`.
- Created `randominit()` (now called `init()`) in `utils.py`.


## [v3.04] - init(), generate_dataset()  |  30/03/26
- Created `generate_dataset()` in `utils.py`.
- Changed `randominit()` to `init()`.
- Changed code in `init()`.


## [v3.05] - Trainer Class  |  18/04/26
- Finished `Trainer` Class in `trainer.py`.